...
test("per-key stats combine correct hits and misses", () => {
  ...
  const a = r.topKeys.find((s) => s.key === "a");
...
test("upper and lower case fold onto the same key", () => {
  ...
  expect(r.topKeys.map((s) => s.key)).toEqual(["t"]);
  expect(r.topKeys[0]).toMatchObject({
    key: "t",
    correct: 5,
    miss: 3,
    total: 8,
  });
  expect(r.topKeys[0].accuracy).toBeCloseTo(5 / 8, 5);
});
...
test("keys rank by how often they came up, tie-break by accuracy", () => {
  ...
  expect(r.topKeys.map((s) => s.key)).toEqual(["a", "e", "x", "z"]);
});
...
test("keys with the same volume are ordered by accuracy, then name", () => {
  ...
  expect(r.topKeys.map((s) => s.key)).toEqual(["c", "b", "a"]);
});

test("the ranking is truncated to 10 keys", () => {
  const keyCorrect: Record<string, number> = {};
  for (let i = 0; i < 12; i++) {
    keyCorrect[String.fromCharCode(97 + i)] = 12 - i;
  }
  const r = computeScore(78, 0, keyCorrect, {});
  expect(r.topKeys).toHaveLength(10);
  expect(r.topKeys[0].key).toBe("a");
  expect(r.topKeys.at(-1)?.key).toBe("j");
});
